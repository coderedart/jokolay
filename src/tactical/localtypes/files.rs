...
use crate::{fm::{FileManager, VID}, tactical::{localtypes::{category::{CatSelectionTree, IMCategory, MCIndexTree}, marker::{MarkerTemplate, POI}, trail::Trail}, xmltypes::{
        xml_category::{XMLMarkerCategory, OverlayData},
        xml_marker::XMLPOI,
        xml_trail::XMLTrail,
    }}};
...
impl MarkerFile {
    pub fn parse_marker_file(pack_path: VID, fm: &FileManager, file_path: VfsPath, global_cats: &mut Vec<IMCategory>, global_pois: &mut HashMap<Uuid, POI>, global_trails: &mut HashMap<Uuid, Trail>, name_id_map: &mut HashMap<String, usize>, mfiles: &mut Vec<MarkerFile>, cstree: &mut Vec<CatSelectionTree> ) {
        let xml_file = file_path.open_file()
                    .map_err(|e| {
                        log::error!(
                ...
                if let Some(p) = od.pois {
                    ...
                    if let Some(vt) = p.trail {
                        uuid_trail_vec = Trail::get_vec_uuid_trail(vt, global_trails, pack_path, global_cats, fm);
                    }
                }
