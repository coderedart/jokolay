...
use crate::{client::{JokoClient, tc::{ASSETS_FOLDER_NAME, LOG_FILE_NAME}}, config::JokoConfig, core::JokoCore};
...
pub fn real_main() -> bool {
    let (mut config, assets_path) = setup().map_err(|e| {
...
pub fn setup() -> anyhow::Result<(JokoConfig, PathBuf)> {
    let assets_path = match std::env::var("JOKO_ASSETS") {
        Ok(ap) => ap,
        Err(_) => {
            std::env::current_dir().context("failed to get current_dir()")?
    ...
    if !config_file_path.exists() {
        ...
        let cf = std::fs::File::create(&config_file_path).context("could not create joko_config.json")?;
    ...
    let config: JokoConfig = serde_json::from_reader(std::io::BufReader::new(config_file)).context("error when trying to parse config file. please delete joko_config.json")?;
    
    let log_file_path = assets_path.join(LOG_FILE_NAME);
    log_init(
        log::LevelFilter::from_str(&config.term_log_level).context("failed to deserialize term log level from config")?,
        log::LevelFilter::from_str(&config.file_log_level).context("failed to deserialize file log level from config")?,
        log_file_path.clone(),
