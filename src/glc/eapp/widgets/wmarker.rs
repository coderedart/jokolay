...
use crate::{glc::{gltypes::texture::Texture, tactical::{scene::{MarkerNode, SceneNode}, xmltypes::{
...
impl MarkersWindow {
    ...
    pub fn add_widgets_to_ui(&mut self, ctx: &CtxRef, mcache: Arc<Mutex<Option<MumbleCache>>>) {
        Window::new(&self.name).show(&ctx, |ui| {
            ...
            ui.label(format!("categories: {}, markers: {}, trails: {}", self.marker_categories.len(), self.markers.len(), self.trails.len()));
                ...
                if let Some(c) = mc.as_ref() {
                    if let Some(link) = c.link.as_ref() {
                            ...
                            self.markers.get(&link.identity.as_ref().unwrap().map_id)
                        {
                            ...
                            let camera_position = Vec3::new(link.f_camera_position_x, link.f_camera_position_y, link.f_camera_position_z);
                            let center = camera_position + Vec3::new(link.f_camera_front_x, link.f_camera_front_y, link.f_camera_front_z);
                            let view = nalgebra_glm::look_at_lh(&camera_position,
                             &center,
                              &Vec3::new(0.0, 1.0, 0.0));
                            let projection = nalgebra_glm::perspective_fov_lh(link.identity.as_ref().unwrap().fov, 800.0, 600.0, 0.1, 30000.0);
                            let vp = projection * view;
                            unsafe {
                                self.marker_scene.gl.active_texture(glow::TEXTURE0 );
                                self.t.bind();
                                    self.marker_scene.gl.clear_color(0.0, 0.0, 0.0, 0.0);
                                    self.marker_scene.gl.clear(glow::COLOR_BUFFER_BIT | glow::DEPTH_BUFFER_BIT | glow::STENCIL_BUFFER_BIT);
                                
                            }
                            self.marker_scene.draw(Some(&nodes), vp, camera_position,
                             Vec3::new(link.f_avatar_position_x, link.f_avatar_position_y, link.f_avatar_position_z));
                        }
                    }
                }
            }
