...
use crate::{glc::tactical::{scene::{MarkerNode, SceneNode}, xmltypes::{
                ...
                if let Some(c) = mc.as_ref() {
                    if let Some(link) = c.link.as_ref() {
                            ...
                            self.markers.get(&link.identity.as_ref().unwrap().map_id)
                        {
                            ...
                            let vp = nalgebra_glm::look_at_lh(&make_vec3(&[link.f_camera_position_x, link.f_camera_position_y, link.f_camera_position_z]),
                             &(make_vec3(&[link.f_camera_position_x, link.f_camera_position_y, link.f_camera_position_z]) + make_vec3(&[link.f_camera_front_x, link.f_camera_front_y, link.f_camera_front_z])),
                              &make_vec3(&[0.0, 1.0, 0.0]));
                            self.marker_scene.draw(Some(&nodes), vp, make_vec3(&[link.f_camera_position_x, link.f_camera_position_y, link.f_camera_position_z]),
                             make_vec3(&[link.f_avatar_position_x, link.f_avatar_position_y, link.f_avatar_position_z]));
                        }
                    }
                }
            }
