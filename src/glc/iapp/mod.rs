...
use glow::{Context as _, HasContext};
...
impl ImguiInterface {
    pub fn new(gl: Rc<glow::Context>, overlay_window: Rc<RefCell<GlfwWindow>>) -> Self {
        ...
        attach_window( ctx.borrow_mut().io_mut(), &mut overlay_window.borrow_mut().window.borrow_mut());
        ImguiInterface {
            ctx,
            painter,
            overlay_window,
    ...
    pub fn update(&self) -> anyhow::Result<()> {
        ...
        }
        {
            ...
            overlay_window.borrow_mut().send_events_to_imgui(ctx.io_mut());
            prepare_frame(ctx.io_mut(), &mut overlay_window.borrow_mut().window.borrow_mut()).unwrap();
