...
impl EguiInterface {
    ...
    pub fn upload_user_texture(&self, pixels: &[u8], width: u32, height: u32) -> TextureId {
        let new_texture = Texture::new(self.painter.borrow().gl.clone());
        new_texture.bind();
    ...
    pub fn update(&self, input: RawInput) -> anyhow::Result<()> {
        ...
        let _gl = self.overlay_window.borrow().get_gl_context();
        ...
        if ctx.borrow().wants_pointer_input() || ctx.borrow().wants_keyboard_input() {
            overlay_window
                .borrow()
                .window
                .borrow_mut()
        ...
        // egui::CentralPanel::default().show(&ctx.borrow(), |ui| {
