...
impl MarkerTemplate {
    ...
    pub fn update_from_element(&mut self, ele: &Node) {
        for attr in ele.attributes() {
            let attr_name = attr.name();
            let attr_value = attr.value();
            let parsed_properly = match attr_name.trim().to_lowercase().as_str() {
                // "alpha" => {
                ...
                // }
                "iconfile" => {
                    self.icon_file = Some(attr_value.to_lowercase());
                    true
                }
                // "iconsize" => {
                ...
                // }
                "texture" => {
                    self.texture = Some(attr_value.to_lowercase());
                    true
                }
                "trailfile" => {
                    self.trail_data_file = Some(attr_value.to_lowercase());
                    true
                }
                // "trailscale" => {
                //     if let Ok(scale) = attr_value.parse() {
