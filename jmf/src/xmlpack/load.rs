...
pub fn xml_to_json_pack(pack_dir: &Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut trail_path_id: BTreeMap<String, u16> = BTreeMap::default();
    let mut trail_pos: BTreeMap<u16, ([f32; 3], u16)> = BTreeMap::default();
...
fn parse_recursive_mc(
    ...
    template.override_from_element(ele)?;
...
impl MarkerTemplate {
    pub fn override_from_element(
        ...
        if let Some(achievement_id) = ele.get_attr("achievementId") {
            self.achievement_id = Some(achievement_id.parse()?);
        }
        if let Some(achievement_bit) = ele.get_attr("achievementBit") {
            self.achievement_bit = Some(achievement_bit.parse()?);
        }
        if let Some(alpha) = ele.get_attr("alpha") {
            self.alpha = Some(alpha.parse()?);
        }
        ...
        if let Some(ff) = ele.get_attr("fadeFar") {
            self.fade_far = Some(ff.parse()?);
        }
        if let Some(fnear) = ele.get_attr("fadeNear") {
            self.fade_near = Some(fnear.parse()?);
        }
        if let Some(has_countdown) = ele.get_attr("hasCountdown") {
            self.has_countdown = Some(has_countdown.parse()?);
        }
        ...
        for mt in ele.children() {
            match mt.tag().name() {
                "POI" => {
                    ...
                    template.override_from_element(mt)?;
                    ...
                    let toggle_cat = template.toggle_cateogry.map(|full_name| fullnames_to_catid.get(&full_name).copied()).flatten();
